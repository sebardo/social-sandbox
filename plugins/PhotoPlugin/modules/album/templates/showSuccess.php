...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        params.userId=           '<?php echo $sesUser->getId(); ?>';
        params.albumUserId=      '<?php echo $user->getId(); ?>';
        //        params.debugMode=true;

        var galery=new Galery(params);
        galery.init(<?php echo $album->getId(); ?>);
    });
</script>
<div id="galeryContainer">
    <ul>
        <li id="photos">
            <?php include_component("photo", "list", array('user' => $user, 'album' => $album)); ?>
        </li>
    </ul>
    <?php if (sfConfig::get('app_photo_favlikeable')) : ?>
        <?php include_component('favlike', 'favlikes', array('object' => $album, 'model' => 'album_photo')) ?>
    <?php endif; ?>
    <?php if (sfConfig::get('app_photo_commentable')) : ?>
        <?php use_javascript('/PubsPlugin/js/comment.js') ?>
        <?php include_component('comment', 'comments', array('object' => $album, 'model' => 'album_photo', 'datos' => $user)) ?>
    <?php endif; ?>
</div>
