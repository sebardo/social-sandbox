...
sfGmapWidgetWidget.prototype.init = function() {
    ...
    this.relLng      = jQuery("#" + this.options.relLongitude);
    this.relLat      = jQuery("#" + this.options.relLatitude);
    this.relAddress  = jQuery("#" + this.options.relAddress);
    ...
    GEvent.addListener(this.map, "move", function() {
        ...
        this.sfGmapWidgetWidget.lng.val(center.lng().toString());
        this.sfGmapWidgetWidget.lat.val(center.lat().toString());
    });
...
sfGmapWidgetWidget.prototype.fillFields = function(){
    this.relLat.val(this.lat.val());
    this.relLng.val(this.lng.val());
    this.relAddress.val(this.address.val());
}
...
sfGmapWidgetWidget.reverseLookupCallback = function(response)
{
    ...
    widget.lat.val(place.Point.coordinates[1].toString());
    widget.lng.val(place.Point.coordinates[0].toString());
    widget.fillFields();
}
