...
class sfResizedFile extends sfValidatedFile
{
	...
	public function save($file = null, $fileMode = 0666, $create = true, $dirMode = 0777)
	{
		...
		if ($file[0] != '/' && $file[0] != '\\' && !(strlen($file) > 3 && ctype_alpha($file[0]) && $file[1] == ':' && ($file[2] == '\\' || $file[2] == '/')))
		{
			...
			$smallFile = $this->path.DIRECTORY_SEPARATOR.'thumb'.DIRECTORY_SEPARATOR.$file;
			$mediumFile = $this->path.DIRECTORY_SEPARATOR.'medium'.DIRECTORY_SEPARATOR.$file;
			$bigFile = $this->path.DIRECTORY_SEPARATOR.'big'.DIRECTORY_SEPARATOR.$file;
		...
		$thumbdir = $directory.DIRECTORY_SEPARATOR.'thumb';
		$mediumdir = $directory.DIRECTORY_SEPARATOR.'medium';
		$bigdir = $directory.DIRECTORY_SEPARATOR.'big';
		
		//large image directorie
		...
		//thumbnail directorie
		if (!is_readable($thumbdir))
		{
			if ($create && !mkdir($thumbdir, $dirMode, true))
			{
				// failed to create the directory
				throw new Exception(sprintf('Failed to create file upload directory "%s".', $directory));
			}
			...
			// chmod the directory since it doesn't seem to work on recursive paths
			chmod($thumbdir, $dirMode);
		}
		
		//big image directorie
		if (!is_readable($bigdir))
		{
			if ($create && !mkdir($bigdir, $dirMode, true))
			{
				// failed to create the directory
				throw new Exception(sprintf('Failed to create file upload directory "%s".', $bigdir));
			}

			// chmod the directory since it doesn't seem to work on recursive paths
			chmod($bigdir, $dirMode);
		}
		
		//medium image directorie
		if (!is_readable($mediumdir))
		{
			if ($create && !mkdir($mediumdir, $dirMode, true))
			{
				// failed to create the directory
				throw new Exception(sprintf('Failed to create file upload directory "%s".', $directory));
			}
		...
		{
			// the directory isn't writable
			throw new Exception(sprintf('File upload path "%s" is not writable.', $thumbdir));
		}
		
		if (!is_writable($mediumdir))
		{
			// the directory isn't writable
			throw new Exception(sprintf('File upload path "%s" is not writable.', $mediumdir));
		}
		
		if (!is_writable($bigdir))
		{
			// the directory isn't writable
			throw new Exception(sprintf('File upload path "%s" is not writable.', $bigdir));
		}


		// copy the temp file to the destination file
		$thumbnail = new sfThumbnail(158, null, true, true, 85/* , 'sfGDAdapterCuttingOff' */);
		$thumbnail->loadFile($this->getTempName());
		$thumbnail->save($smallFile, 'image/jpeg');
		
		//medium file
		$thumbnail = new sfThumbnail(202, null, true, true, 85/* , 'sfGDAdapterCuttingOff' */);
		$thumbnail->loadFile($this->getTempName());
		$thumbnail->save($mediumFile, 'image/jpeg');
		
        //aqui guardo la imagen original
		copy($this->getTempName(), $bigFile);
		
		//$thumbnail = new sfThumbnail(600, null, true, true, 85/* , 'sfGDAdapter' */);
		//$thumbnail->loadFile($this->getTempName());
		//$thumbnail->save($bigFile, 'image/jpeg');

		// chmod our file
		chmod($smallFile, $fileMode);
		//chmod($file, $fileMode);
		chmod($mediumFile, $fileMode);
		chmod($bigFile, $fileMode);
...
class sfColorFile extends sfValidatedFile
{
  public function save($file = null, $fileMode = 0666, $create = true, $dirMode = 0777)
  {
    if (null === $file)
